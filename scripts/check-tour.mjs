// Simulate the scripted tour (apartments/<id>/tour.js) against the real apartment:
...
//
//   node scripts/check-tour.mjs                 # every apartment that has a tour
//   node scripts/check-tour.mjs l2-39           # just this one
//   node scripts/check-tour.mjs l2-39 --dump    # also list placed pieces / doors for waypoint planning
//   node scripts/check-tour.mjs l2-39 --trace   # print the body position after every step
...
import { apartmentArgs, loadApartment } from './lib/apartments.mjs';

async function check(id) {
  const { apartment, scene, piecesById, tour } = await loadApartment(id);
  if (!tour) {
    console.log(`${id}: no tour.js - skipped`);
    return 0;
  }
  ...
  return warnings.length + stillOpen.length;
}

let problems = 0;
const ids = apartmentArgs();
for (const id of ids) {
  if (ids.length > 1) console.log(`\n== ${id} ==`);
  problems += await check(id);
}
