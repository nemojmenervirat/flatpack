// Audit for apartments/<id>/rooms.json: verifies every room area with exact
// rectangle arithmetic, independent of the app code.
//
//   node scripts/check-areas.mjs           # every apartment
//   node scripts/check-areas.mjs l2-39     # just this one
//
// Checks:
...
function check({ id, apartment, rooms }) {
  if (!rooms.length) {
    console.log(`${id}: no rooms.json - skipped`);
    return 0;
  }
  const EPS = 0.5;
  ...
  console.log(`${id}: rooms.json audit —`, rooms.length, 'rooms\n');
  let total = 0;
...
let failures = 0;
for (const id of apartmentArgs()) failures += check(await loadApartment(id));
